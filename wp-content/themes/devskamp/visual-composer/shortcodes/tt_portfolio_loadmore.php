<?php
    ...
    if ($tt_atts['portfolio_style'] == 'title-style') {
...
?>
    ...
    <div class="portfolio-container <?php echo esc_attr($tt_atts['el_class'].' '.$css_class);?>">
            ...
            <ul class="portfolio-filter list-inline <?php echo esc_attr($tt_atts['filter_style'].' '.$tt_atts['filter_alignment'].' '.$tt_atts['filter_color']); ?>">
                ...
                <?php foreach ( $tax_terms as $term_id ) :
                    ...
                    if (! empty( $term ) && ! is_wp_error( $term )) : ?>
                        <li class="waves-effect waves-light" data-filter=".<?php echo esc_attr($term->slug); ?>">
        ...
        <div class="tt-grid portfolio row portfolio-loadmore <?php echo esc_attr($style_class.' '.$tt_atts['grid_padding']); ?>">
            <?php
            ...
            <?php if ( $query->have_posts() ) : ?>
                ...
                ?>
                    <div class="tt-item portfolio-item col-md-<?php echo esc_attr($tt_atts['grid_column']);?> col-sm-6 col-xs-12 <?php echo esc_attr(implode( ' ', $term )); ?>">
                        ...
                        <?php if ($tt_atts['portfolio_style'] == 'boxed-style' || $tt_atts['portfolio_style'] == 'title-style' || $tt_atts['portfolio_style'] == 'masonry-style'): ?>
                            <div class="portfolio-wrapper">
                                ...
                                <?php if ($tt_atts['portfolio_style'] == 'title-style') : ?>
        ...
        <?php if (function_exists('tt_portfolio_post_type')) :
            $allposts = wp_count_posts('tt-portfolio');
            $postlimit = $tt_atts['post_limit'];

            if ($allposts->publish > $postlimit && $postlimit != -1) : ?>
                <div class="row loadmore-btn-wrap text-center">
                    <div class="col-md-12">
                        ...
                        data-style="<?php echo esc_attr($tt_atts['portfolio_style']);?>" 
                        data-padding="<?php echo esc_attr($tt_atts['grid_padding']);?>" 
                        data-title="<?php echo esc_attr($tt_atts['title_visibility']);?>" 
                        data-popup="<?php echo esc_attr($tt_atts['popup_button_visibility']);?>" 
                        data-like="<?php echo esc_attr($tt_atts['like_visibility']);?>" 
                        data-category="<?php echo esc_attr($tt_atts['category_visibility']);?>" 
                        data-link_button="<?php echo esc_attr($tt_atts['link_button_visibility']);?>" 
                        data-word_limit="<?php echo esc_attr($tt_atts['word_limit']);?>" 
                        data-button_text="<?php echo esc_attr($tt_atts['button_text']);?>" href="#"><?php echo esc_html($tt_atts['loadmore_text']); ?></a>
                    </div>
                </div>
            <?php endif;
        endif; ?>
